...
const Icon: React.FC<IconProps> = ({ name, iconHeight, iconWidth, setSelectedIcon }) => {
  const IconComponent = icons[name];
  return IconComponent ? <img onClick={()=>setSelectedIcon(name)} src={IconComponent} className={`hover:outline outline-2 outline-offset-2 cursor-pointer bg-blue-500 p-2 rounded-md min-w-[${iconWidth}] min-h-[${iconHeight}]`} alt={name} width={iconWidth} height={iconHeight} /> : <span>Icon not found</span>;
};
